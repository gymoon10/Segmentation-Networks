...
def main():
    ...
    for epoch in range(start_epoch, args['n_epochs']):
        ...
        for i, sample in enumerate(tqdm(train_dataset_it)):
            ...
            model.train()
            pred = model(image)
            loss1 = bce_iou_loss(pred[0], label.unsqueeze(1).type(torch.float))
            loss2 = bce_iou_loss(pred[1], label.unsqueeze(1).type(torch.float))
            loss3 = bce_iou_loss(pred[2], label.unsqueeze(1).type(torch.float))
            loss4 = bce_iou_loss(pred[3], label.unsqueeze(1).type(torch.float))
            loss5 = bce_iou_loss(pred[4], label.unsqueeze(1).type(torch.float))
        ...
        with torch.no_grad():
            for i, sample in enumerate(tqdm(val_dataset_it)):
                ...
                loss = bce_iou_loss(res, label.unsqueeze(1).type(torch.float))
